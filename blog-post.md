...
![OpenObserve dashboard home page after logging in, showing the Logs, Traces, Metrics and other navigation options in the sidebar](<img/screenshot-openobserve-home.png>)
...
![OpenObserve Data Sources page showing Traces (OpenTelemetry) selected, with OTLP gRPC config displaying the Authorization header and organization fields](<img/screenshot-openobserve-datasources.png>)
...
![Terminal showing Spring Boot console output with populated traceId and spanId on a live request log line](<img/screenshot-terminal-traceid.png>)
...
![OpenObserve Logs view with a trace_id filter applied, showing all structured log records for a single request](<img/screenshot-logs-trace-filter.png>)
...
![Side-by-side view of OpenObserve Logs filtered by trace_id and the matching trace waterfall open in the Traces view](<img/screenshot-correlation-logs.png>)

![Side-by-side view of OpenObserve Logs filtered by trace_id and the matching trace waterfall open in the Traces view](<img/screenshot-correlation-trace.png>)*Log-trace correlation in action - the same trace_id connects a log record in the Logs view to the full request waterfall in the Traces view*
